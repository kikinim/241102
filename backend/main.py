...
def calculate(data: CalculationRequest):
    ...
    try:
        ...
        conductance = int(input('conductance 를 입력하세요'))
        pump = input('pump를 입력하세요').upper()
       
        target_pressure=int(input('target_pressure를 입력하세요'))
        target_flowrate=int(input('target_flow를 입력하세요'))
